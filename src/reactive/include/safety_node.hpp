...
class SafetyNode : public rclcpp::Node {
   ...
   private:
    /// @brief Callback invoked on new odometry data; updates the current longitudinal speed used
    /// for iTTC.
    ...
    /// @brief Callback invoked on new joystick input; updates deadman button state and last-seen
    /// time.
    ...
    /// @brief Callback invoked when a controller node publishes a drive command; forwards it to
    /// /drive unless gated by estop or deadman.
    /// @param drive_msg Shared pointer to the incoming raw AckermannDriveStamped message.
    void drive_raw_callback(
