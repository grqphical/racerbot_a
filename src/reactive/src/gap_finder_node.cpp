...
GapFinderNode::GapFinderNode() : Node("gap_finder_node") {
    ...
    laser_scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "scan", 10, std::bind(&GapFinderNode::lidar_callback, this, std::placeholders::_1));
...
vector<float> GapFinderNode::preprocess_lidar(
    ...
    for (size_t i = 0; i < ranges.size(); ++i) {
        ...
        if (std::abs(prev_reading - current_reading) > disparity_threshold_) {
            ...
            for (size_t j = start; j < end; ++j) {
                ranges[j] =
                    std::min(ranges[j], closer_range);  // don't overwrite an already-closer point
            }
        }
...
std::pair<int, int> GapFinderNode::find_furthest_gap(
    const sensor_msgs::msg::LaserScan::ConstSharedPtr scan_msg, vector<float> &ranges) {
    int best_start = -1;
    int best_end = -1;
    ...
    while (i < ranges.size()) {
        ...
        float min_range_in_gap =
            *std::min_element(ranges.begin() + gap_start, ranges.begin() + gap_end);
